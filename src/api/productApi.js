...
export const createProduct = (form) =>
    http.post('/admin/products', form).then(res => res.data); // { productId }
...
export const getProductImages = (productId) =>
    http.get(`/admin/products/${productId}/images`).then(res => res.data);

export const uploadProductImage = (productId, formData) =>
    http.post(`/admin/products/${productId}/images`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
    }).then(res => res.data);

export const deleteProductImage = (productId, imageId, imagePath) =>
    http.delete(`/admin/products/${productId}/images/${imageId}`, { params: { imagePath } });

export const updateImageOrder = (productId, items) =>
    http.put(`/admin/products/${productId}/images/order`, items);

/* 동영상 */
export const updateProductVideoUrl = (productId, videoUrl) =>
    http.patch(`/admin/products/${productId}/video-url`, { videoUrl });

export const uploadProductVideo = (productId, file) => {
    const fd = new FormData();
    fd.append('file', file);
    return http.post(`/admin/products/${productId}/video`, fd, {
        headers: { 'Content-Type': 'multipart/form-data' }
    }).then(res => res.data);
};

export const deleteProductVideo = (productId) =>
    http.delete(`/admin/products/${productId}/video`);

/* 첨부파일 */
export const getProductFiles = (productId) =>
    http.get(`/admin/products/${productId}/files`).then(res => res.data);

export const uploadProductFile = (productId, formData) =>
    http.post(`/admin/products/${productId}/files`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
    }).then(res => res.data);

export const deleteProductFile = (productId, fileId) =>
    http.delete(`/admin/products/${productId}/files/${fileId}`);
