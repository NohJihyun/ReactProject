...
import * as api from '../../../api/domesticApi';
...
export default function DomesticItineraryTab({ productId, onComplete }) {
    ...
    const loadDetail = async () => {
        setDetailLoad(true);
        try {
            const data = await api.getDomesticDetail(productId);
    ...
    const handleSaveDetail = async () => {
        ...
        try {
            const saved = await api.saveDomesticDetail(productId, detail);
...
function ItinerarySection({ productId, onAfterSave }) {
    ...
    const load = async () => {
        setLoading(true);
        try { setItineraries(await api.getDomesticItineraries(productId)); }
        finally { setLoading(false); }
    };

    const silentLoad = async () => {
        try { setItineraries(await api.getDomesticItineraries(productId)); } catch {}
    };
    ...
    const handleSave = async () => {
        ...
        try {
            if (editTarget) {
                await api.updateDomesticItinerary(productId, editTarget.id, params);
                setSnack({ open: true, msg: '일정이 수정되었습니다.' });
                closeDialog();
                await load();
            } else {
                const created = await api.createDomesticItinerary(productId, params);
                ...
                for (let i = 0; i < validSchedules.length; i++) {
                    ...
                    const createdSchedule = await api.addDomesticSchedule(productId, created.id, sp);
                    for (const img of (validSchedules[i].pendingImages || [])) {
                        const fd = new FormData();
                        fd.append('file', img.file);
                        await api.uploadDomesticScheduleImage(productId, created.id, createdSchedule.id, fd);
                        URL.revokeObjectURL(img.previewUrl);
                    }
                }
                const fresh = await api.getDomesticItineraries(productId);
    ...
    const handleDeleteItinerary = async (id) => {
        ...
        await api.deleteDomesticItinerary(productId, id);
        await load();
    };
    ...
    const handleAddSchedule = async () => {
        ...
        try {
            await api.addDomesticSchedule(productId, activeItineraryId, params);
    ...
    const handleUpdateSchedule = async (scheduleId) => {
        ...
        try {
            await api.updateDomesticSchedule(productId, activeItineraryId, scheduleId, params);
            setEditingScheduleId(null);
            await silentLoad();
        } finally { setScheduleSaving(false); }
    };
    ...
    const handleDeleteSchedule = async (scheduleId) => {
        ...
        await api.deleteDomesticSchedule(productId, activeItineraryId, scheduleId);
        await silentLoad();
    };
                                                        ...
                                                        <input type="file" accept="image/*" multiple hidden ref={editSchedImgRef}
                                                            onChange={async e => {
                                                                ...
                                                                for (const file of Array.from(e.target.files)) {
                                                                    ...
                                                                    await api.uploadDomesticScheduleImage(productId, activeItineraryId, s.id, fd);
                                                                }
                                                                e.target.value = '';
                                                                await silentLoad();
                                                            }} />
                                                            ...
                                                            {s.images.map(img => (
                                                                <Box key={img.id} sx={{ position: 'relative' }}>
                                                                    ...
                                                                    <IconButton size="small" color="error"
                                                                        sx={{ position: 'absolute', top: 1, right: 1, bgcolor: 'rgba(255,255,255,0.85)', p: '2px' }}
                                                                        onClick={async () => {
                                                                            await api.deleteDomesticScheduleImage(productId, activeItineraryId, s.id, img.id);
                                                                            await silentLoad();
                                                                        }}>
                                                                        <DeleteIcon sx={{ fontSize: 12 }} />
                                                                    </IconButton>
