        ...
        try {
            if (editTarget) {
                ...
                for (let i = 0; i < validSchedules.length; i++) {
                    ...
                    const createdSchedule = await api.addSchoolTripSchedule(productId, created.id, sp);
                    for (const img of (validSchedules[i].pendingImages || [])) {
                        const fd = new FormData();
                        fd.append('file', img.file);
                        await api.uploadSchoolTripScheduleImage(productId, created.id, createdSchedule.id, fd);
                        URL.revokeObjectURL(img.previewUrl);
                    }
                }
                const fresh = await api.getSchoolTripItineraries(productId);
                                                        ...
                                                        <input type="file" accept="image/*" multiple hidden ref={editSchedImgRef}
                                                            onChange={async e => {
                                                                ...
                                                                for (const file of Array.from(e.target.files)) {
                                                                    ...
                                                                    await api.uploadSchoolTripScheduleImage(productId, activeItineraryId, s.id, fd);
                                                                }
                                                                e.target.value = '';
                                                                await silentLoad();
                                                            }} />
                                                            ...
                                                            {s.images.map(img => (
                                                                <Box key={img.id} sx={{ position: 'relative' }}>
                                                                    ...
                                                                    <IconButton size="small" color="error"
                                                                        sx={{ position: 'absolute', top: 1, right: 1, bgcolor: 'rgba(255,255,255,0.85)', p: '2px' }}
                                                                        onClick={async () => {
                                                                            await api.deleteSchoolTripScheduleImage(productId, activeItineraryId, s.id, img.id);
                                                                            await silentLoad();
                                                                        }}>
                                                                        <DeleteIcon sx={{ fontSize: 12 }} />
                                                                    </IconButton>
